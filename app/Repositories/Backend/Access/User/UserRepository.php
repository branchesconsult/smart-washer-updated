...
class UserRepository extends BaseRepository
{
     ...
     * @return mixed
     */
    public function getForDataTable($status = 1, $trashed = false, $laundry_id = 0, $ownerOrDriver = false)
    {
        /**
         * Note: You must return deleted_at or the User getActionButtonsAttribute won't
         * be able to differentiate what buttons to show for each row.
         */
        if ($laundry_id == 0 || !$ownerOrDriver) {
            ...
            $dataTableQuery = User::whereHas('laundries', function ($q) use ($laundry_id, $ownerOrDriver) {
                $q->where('laundries.id', $laundry_id)->where('relation', $ownerOrDriver);
            });
            //dd($dataTableQuery->get()->toArray());
        }
