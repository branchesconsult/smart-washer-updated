...
class DriverOrderController extends Controller
{
    ...
    public function getDriverOrders(GetOrdersRequest $request)
    {
        ...
        $driver_id = isset($_REQUEST['driver_id']) ? $_REQUEST['driver_id'] : \Auth::id();
        $driver = LaundryUser::where('user_id',$driver_id)->first();

        $location = Location::find($driver->location_id);
        ...
        $orders = Order::with(['detail' => function ($q) {
            ...
            ->where('driver_id', $driver_id)//\Auth::id())
        ...
        foreach ($data['orders'] as $key => $val) {
            $formatRes[$key] = $val;
            $val['currency_code'] = $location->currency_code;
            if (!empty($val['detail'])) {
