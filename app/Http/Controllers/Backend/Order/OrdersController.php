...
//        dd($order['pickup_time']);
        ...
        $geocode=file_get_contents('https://maps.googleapis.com/maps/api/geocode/json?latlng='.$order->lat_lng->getLat().','.$order->lat_lng->getLng().'&sensor=false&key=AIzaSyBiISc2o96q9Jvr_l_VQ86aqCW6UAsYALw');
        ...
        if(empty($location)){
            $location = Location::where('currency_code','SAR')->first();
            $location_id = $location->id;
        }else{
            $location_id = $location->id;
        }

        $products = Product::with(['services' => function ($q2) use ($location_id) {
            $q2->select('service_id', 'price', 'discount_price', 'name', 'laundry_price')->where('location_id',$location_id);
        }])
...
//        $orderMain->total = $newOderFinalTotal;
    ...
    public function deleteOrderDetail(Request $request)
    {
        ...
        $orderSummery->save();
        $this->updateMainOrderFromDetail($orderId);
        $orderUpdate = Order::where('id',$orderId)->with('detail', 'user', 'status', 'driver')->get();
        return response()->json([
            'success' => true,
            'order' => $orderUpdate
        ]);
    ...
    public function showReciept(Request $request)
    {
        ...
        $deviceToken = array_column(Device::where('deviceable_id', $order['user_id'])
            ->get(['device_token'])
            ->toArray(), 'device_token');
        return view('emails.order-receipt-pdf', compact('order'));
        //return loadView('emails.order-receipt-pdf', ['order' => $order]);
    }
