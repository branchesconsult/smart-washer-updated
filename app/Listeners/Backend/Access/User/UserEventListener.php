    ...
    public function onCreated($event)
    {
        if (\Auth::check()) {
            history()->withType($this->history_slug)
                ...
                ->log();
            $mail = new SendEmail();
            $data['first_name'] = $event->user->full_name;
            $data['confirmation_code'] = 'wddasdasd';
            $data['email'] = $event->user->email;
            $data['password'] = request()->get('password');
            $mail->sendWithTemplate($data, 2);
        } else {//User register by him self
            $mail = new SendEmail();
            $data['first_name'] = $event->user->full_name;
            $data['confirmation_code'] = 'wddasdasd';
            $data['email'] = $event->user->email;
            $mail->sendWithTemplate($data, 1);
        }
    }
